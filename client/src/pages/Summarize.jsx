...
const Summarize = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("file", input); // ✅ common field name
...
/>
            ...
            <button
              ...
              disabled:opacity-50"
            >
              {loading ? ( <> <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> <span>Generating...</span> </> ) : ( <> <Zap className="w-5 h-5" /> <span>Summarize File Data</span> </> )}
            </button>
          </form>
          ...
          <div className="glass rounded-2xl p-6 border border-pink-500/20 flex flex-col min-h-[500px] max-h-[700px]">
            ...
            {!content ? ( <div className="flex-1 flex justify-center items-center"> <div className="text-center"> <div className="w-16 h-16 rounded-full bg-gradient-to-br from-red-500/20 to-pink-500/20 flex items-center justify-center mx-auto mb-4"> <Upload className="w-8 h-8 text-gray-500" /> </div> <p className="text-sm text-gray-400"> Upload file and generate summarized data </p> </div> </div> ) : ( <div className="flex-1 overflow-y-auto"> <div className="reset-tw prose prose-invert max-w-none"> <Markdown>{content}</Markdown> </div> </div> )} </div> </div> </div> </div> ); }; export default Summarize;